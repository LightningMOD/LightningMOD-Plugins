    ...
    public class NecCommandSkeletonsPlugin : AbstractSkillHandler, ISkillHandler
    {
        ...
        public override void Load(IController hud)
        {
            ...
            CreateCastRule()
                ...
                {
                    bool cast = false;
                    if (Hud.Game.Me.Powers.UsedLegendaryPowers.BloodsongMail?.Active == true)//血歌锁甲
                    {
                        if (Hud.Game.Me.Powers.BuffIsActive(Hud.Sno.SnoPowers.Necromancer_LandOfTheDead.Sno))//亡者领域
                        {
                            if (!isCast)
                            {
                                isCast = true;
            ...
            CreateCastRule()
                ...
                .IfCastingPortal().ThenNoCastElseContinue()
                .IfPrimaryResourceIsEnough(0, ctx => 0).ThenContinueElseNoCast()
                .IfTrue(ctx => Hud.Game.Me.Powers.BuffIsActive(Hud.Sno.SnoPowers.Necromancer_CommandSkeletons.Sno)).ThenNoCastElseContinue()//激活时不继续
                .IfTrue(ctx =>
                {//选中任意怪时
                    var monster = Hud.Game.SelectedMonster2;
                    if (monster == null)
            ...
            CreateCastRule()
                ...
                {//选中任意怪时
                    ...
                    else
                    {
                        ...
                        if (ctx.Skill.GetResourceRequirement() <= 25 && ObsidianRingoftheZodiac)//消耗低于25+刀盾+黄道 = 持续施放
                        {
                            return true;
                        }
                    }
