    ...
    public class NecBoneArmorPlugin : AbstractSkillHandler, ISkillHandler
    {
        ...
        public override void Load(IController hud)
        {
            ...
            CreateCastRule()
                ...
                .IfTrue(ctx => ctx.Skill.Rune == 1 || (ctx.Skill.Rune == 2 && hud.Game.Me.Powers.BuffIsActive(hud.Sno.SnoPowers.ConventionOfElements.Sno) && hud.Game.Me.Powers.BuffIsActive(hud.Sno.SnoPowers.KrysbinsSentence.Sno))).ThenNoCastElseContinue()//限制免疫符文或者（白骨脱臼+元素戒+克利斯宾）时不使用该规则
                .IfTrue(ctx =>
                {
                    var monsters = ctx.Hud.Game.AliveMonsters.Where(m => !m.Invulnerable && !m.Invisible && (m.CentralXyDistanceToMe < 30 + m.RadiusBottom));
                    return monsters.Count() >= (ctx.Hud.Game.Me.Powers.BuffIsActive(476686, 0) ? 2 : 1) || hud.Game.ActorQuery.IsEliteOrBossCloserThan(30, false);//满足周围怪物最低要求的数量
                })
            ...
            CreateCastRule()
                ...
                .IfTrue(ctx =>//优先在爆发元素使用
                {
                    ...
                    var CoeBuffLeftTime = PublicClassPlugin.GetBuffLeftTime(hud, hud.Sno.SnoPowers.ConventionOfElements.Sno, PublicClassPlugin.GetHighestElement(Hud, ctx.Skill.Player, CoeIndex));//获取爆发元素剩余BUFF时间
