namespace Turbo.Plugins.LightningMod
{
    ...
    public class EliteOrBossNearbyCursorTest : AbstractSkillTest
    {
        ...
        internal override SkillTestResult Test(TestContext context)
        {
            ...
            bool Result = context.Hud.Game.AliveMonsters.Any(m =>((m.IsElite && (IncludeMinion ? true : m.Rarity != ActorRarity.RareMinion)) || m.SnoMonster.Priority == MonsterPriority.goblin) && m.FloorCoordinate.XYZDistanceTo(cursor) <= range && !m.Invulnerable && !m.Invisible && !m.Illusion);
