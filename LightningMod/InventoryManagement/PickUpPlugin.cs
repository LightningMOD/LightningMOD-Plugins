namespace Turbo.Plugins.LightningMod
{
    ...
    public class PickUpPlugin : BasePlugin, IKeyEventHandler, IInGameTopPainter, IAfterCollectHandler, INewAreaHandler
    {
        ...
        public void AfterCollect()
        {
            ...
            if (Hud.Game.Items.Where(item => item.IsOnScreen).Count() <= 0) return;
            var Items = Hud.Game.Items.Where(item =>
            {
                if (WhenForceMoveInvalid && Hud.Interaction.IsHotKeySet(ActionKey.Move) && Hud.Interaction.IsContinuousActionStarted(ActionKey.Move))
                    return false;
                if (item.Location != ItemLocation.Floor || !item.IsOnScreen || item.NormalizedXyDistanceToMe > 10 || (item.AccountBound && !item.BoundToMyAccount))
                    return false;
                if (_clickedAnnIds.Contains(item.AnnId))
                    return false;

                if (PickCraft && item.SnoItem.Kind == ItemKind.craft && item.SnoItem.Sno != 1844495708)//材料但不为打孔器时可直接拾取
                    return true;
                ...
                if (PickCraft && item.SnoItem.Sno == 2087837753)//死亡之息
                    return true;
                if (PickCraft && item.SnoItem.Sno == 1844495708 && CalculateStackItemfreeSpace(item, 100))//打孔器
                    return true;
